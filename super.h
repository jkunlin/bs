...
class Maxclique {
	...
	void cut_mcs(const Vertices&, Vertices&, const int pi);
	void color_sort(Vertices&);
	void re_color_sort(Vertices&);
	void re_color_sort_mcs(Vertices&, Vertices&);
	...
	void detect_clique_vertices(Vertices&, Vertices&);
	void expand(Vertices);
	void expand_dyn(Vertices);
	void expand_mcs(Vertices, Vertices);
...
void Maxclique::Vertices::pop_index(const int index) {
...
void Maxclique::detect_clique_vertices(Vertices &R, Vertices &Ra) {
	int max_no = R.end().get_degree();
	int clq_v = -1;
	for (int k = 1; k <= max_no; ++k) {
		if (C[k].size() > 1)
...
void Maxclique::expand_mcs(Vertices R, Vertices Ra) {
	while (R.size()) {
		if (Q.size() + R.end().get_degree() > QMAX.size()) {
			Q.push(R.end().get_i());
			Vertices Rap(R.size());
			cut_mcs(Ra, Rap, R.end().get_i());
			if (Rap.size()) {
				Vertices Rp(R.size());
...
void Maxclique::expand_dyn_mcs(Vertices R, Vertices Ra) {
	S[level].set_i1(S[level].get_i1() + S[level - 1].get_i1() - S[level].get_i2());
	S[level].set_i2(S[level - 1].get_i1());
	while (R.size()) {
		if (Q.size() + R.end().get_degree() > QMAX.size()) {
